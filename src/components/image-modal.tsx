    ...
    >
      ...
      {currentIndex > 0 && (
        <button
          ...
          className='absolute left-4 top-1/2 -translate-y-1/2 text-white/80 hover:text-white p-3 backdrop-blur-sm bg-black/40 rounded-full transition-all duration-200 hover:scale-110 hover:bg-black/60 animate-in fade-in slide-in-from-left-8 z-[60]'
        >
          <ChevronLeft size={32} />
        </button>
      )}
      ...
      {currentIndex < images.length - 1 && (
        <button
          onClick={handleNext}
          className='absolute right-4 top-1/2 -translate-y-1/2 text-white/80 hover:text-white p-3 backdrop-blur-sm bg-black/40 rounded-full transition-all duration-200 hover:scale-110 hover:bg-black/60 animate-in fade-in slide-in-from-right-8 z-[60]'
        >
      ...
      <div className='relative max-h-[90vh] max-w-[90vw] animate-in zoom-in-50 duration-300'>
        <img
          ...
          alt={`Full size view, uploaded by ${toTitleCase(currentImage.uploadedBy)}`}
        ...
        <div className='absolute bottom-4 left-4 bg-black/60 text-white px-3 py-2 rounded-md text-sm backdrop-blur-sm animate-in fade-in slide-in-from-bottom-4 duration-500 z-[60]'>
          Uploaded by {toTitleCase(currentImage.uploadedBy)}
        </div>
      </div>
    </div>
  )
